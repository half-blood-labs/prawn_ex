...
:ok =
  PrawnEx.build(path, fn doc ->
    ...
    |> PrawnEx.Layout.heading("PrawnEx layout phases (A–D)", font_size: 22, gap_after: 10)
    |> PrawnEx.Layout.paragraph(
      "This PDF is generated entirely with PrawnEx.Layout + Layout.Markup. Each section is labelled by phase. Coordinates use the PDF bottom-left origin.",
      font_size: 10,
      line_height: 13,
      gap_after: 10
    )
    |> PrawnEx.Layout.heading("Phase A — Flow primitives", font_size: 15, level: 2, gap_after: 6)
    |> PrawnEx.Layout.paragraph(
    ...
    |> PrawnEx.Layout.heading("Phase B — vstack", font_size: 15, level: 2, gap_after: 6)
    |> PrawnEx.Layout.paragraph(
    ...
    |> PrawnEx.Layout.vstack(
      [
        {:paragraph, "vstack child 1", font_size: 10, gap_after: 2},
        {:paragraph, "vstack child 2", font_size: 10, gap_after: 2},
        {:spacer, 6},
        {:paragraph, "After intra-stack spacer + vstack gap.", font_size: 10, gap_after: 4}
      ],
      gap: 8
    )
    |> PrawnEx.Layout.heading("Phase B — hstack", font_size: 15, level: 2, gap_after: 6)
    ...
    |> PrawnEx.Layout.heading("Phase D — Markup (line DSL)",
      font_size: 15,
      level: 2,
      gap_after: 6
    )
